...
public class ShowCaseDialog extends DialogFragment {
    ...
    public void next() {
        if ((currentTutorIndex + 1) >= tutorsList.size()) {
            ...
            ShowCaseDialog.this.show(this.getActivity(), tag, tutorsList, currentTutorIndex + 1);
        }
    }
    ...
    public boolean hasShown(AppCompatActivity activity, String tag) {
        return ShowCasePreference.hasShown(activity, tag);
    }

    public void show(AppCompatActivity activity, @Nullable String tag, final ArrayList<ShowCaseObject> tutorList) {
        show(activity, tag, tutorList, 0);
    }

    public void show(final FragmentActivity activity, @Nullable String tag, final ArrayList<ShowCaseObject> tutorList, int indexToShow) {
    ...
    public void showLayout(FragmentActivity activity, ShowCaseObject showCaseObject) {
        if (activity == null || activity.isFinishing()) {
            return;
        }
        ...
        if (view == null) {
            ...
            view.post(() -> layoutShowTutorial(view, title, text, showCaseContentPosition,
                    tintBackgroundColor, location, radius));
        }
    }
